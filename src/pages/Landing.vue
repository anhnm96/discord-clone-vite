<template>
  <div class="flex flex-col h-screen mx-auto max-w-7xl px-content">
    <header class="flex justify-between py-8">
      ...
      <div class="flex gap-4">
        <template v-if="!userStore.isAuth">
          <router-link
            to="/login"
            class="px-4 py-2 font-semibold transition border rounded-md hover:bg-purple bg-tertiary text-primary border-primary"
          >
            Login
          </router-link>
          <router-link
        ...
        <router-link
          v-else
          to="/channels/me"
          class="px-4 py-2 transition border rounded-md hover:bg-purple bg-tertiary text-primary border-primary"
        >
          Open App
        </router-link>
      </div>
    ...
    <main class="flex items-center flex-grow">
      <div class="flex flex-col-reverse justify-between gap-10 md:gap-5 md:flex-row">
        <div class="flex flex-col items-start justify-center w-4/5 mx-auto space-y-6 md:w-2/5">
          <h1 class="text-3xl font-bold text-center lg:text-4xl md:text-left text-primary">
            An invite-only place with plenty of room to talk
          </h1>
          <p class="text-xl text-center md:text-left text-primary">
            Discord servers are organized into topic-based channels where you can collaborate, share, and just talk about your day without clogging up a group chat.
          </p>
          <router-link
            to="/register"
            class="px-4 py-2 mx-auto font-semibold text-gray-900 transition rounded-md md:mx-0 hover:bg-blue-dark bg-blue-light"
          >
            Get Started
          </router-link>
          <p class="mx-auto text-sm md:mx-0 text-primary">
        ...
        <div class="flex items-center justify-center w-4/5 mx-auto sm:w-3/5 md:w-1/2">
          <img
